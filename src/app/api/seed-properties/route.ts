...
export async function GET() {
    ...
    for (let i = 0; i < properties.length; i++) {
        ...
        if (propError) {
            results.push({ error: propError, prop: prop.title })
            continue
        }
        ...
        if (imageError) {
            results.push({ error: imageError, prop: prop.title, context: "images" })
        } else {
            results.push({ success: true, prop: prop.title, images: imagesToInsert.length })
        }
    }
